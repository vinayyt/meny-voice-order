...
<title>MENY Bestilling ${orderId}</title>
</head>
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
  ...
  <!-- Header -->
  <tr>
    <td style="background:#D62828;border-radius:12px 12px 0 0;padding:28px 32px;text-align:center;">
      ...
      <div style="color:rgba(255,255,255,0.85);font-size:14px;margin-top:6px;letter-spacing:0.03em;">AI Bestillingsassistent</div>
    </td>
  </tr>

  <!-- Order ID badge -->
  <tr>
    <td style="background:#1F1B14;padding:12px 32px;text-align:center;">
  ...
  <!-- Body -->
  <tr>
    <td style="background:#FBF7EE;padding:32px;">
      ...
      <table width="100%" style="margin-bottom:28px;">
        <tr>
          <td>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#F4EEE2;border-radius:8px;padding:16px;">
              <tr>
                <td style="padding:4px 0;font-size:14px;color:#4A4438;width:80px;">Navn</td>
                <td style="padding:4px 0;font-size:14px;font-weight:600;color:#1F1B14;">${customerName || "—"}</td>
              </tr>
              <tr>
                <td style="padding:4px 0;font-size:14px;color:#4A4438;">E-post</td>
                <td style="padding:4px 0;font-size:14px;color:#1F1B14;">${customerEmail || "—"}</td>
              </tr>
              ...
              ${deliveryAddress ? `<tr><td style="padding:4px 0;font-size:14px;color:#4A4438;">Leveringsadresse</td><td style="padding:4px 0;font-size:14px;color:#1F1B14;">${deliveryAddress}</td></tr>` : ""}
            </table>
          </td>
        </tr>
      </table>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="border-radius:8px;overflow:hidden;border:1px solid #ECE4D2;">
        <thead>
          <tr style="background:#ECE4D2;">
        ...
        ${grandTotal > 0 ? `
        <tfoot>
          <tr style="background:#1F1B14;">
            <td colspan="2" style="padding:14px;font-size:15px;font-weight:700;color:#FBF7EE;">Estimert totalsum</td>
            <td style="padding:14px;text-align:right;font-size:18px;font-weight:900;color:#FFFFFF;">${grandTotal} kr</td>
          </tr>
        </tfoot>` : ""}
      </table>
      ...
      <div style="margin-top:24px;">
        <div style="font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#8A8170;margin-bottom:10px;">Merknad fra kunde</div>
        <div style="background:#F0EEFF;border-left:3px solid #9B8FE8;border-radius:6px;padding:14px 16px;font-size:14px;font-style:italic;color:#4A4438;">"${notes}"</div>
      </div>` : ""}

      <!-- Chat history -->
      ${historyRows ? `
      <div style="margin-top:28px;">
        <div style="font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#8A8170;margin-bottom:10px;">Samtalehistorikk</div>
        <div style="background:#F4EEE2;border-radius:8px;padding:16px;max-height:400px;overflow:hidden;">${historyRows}</div>
      </div>` : ""}
  ...
  <!-- Footer -->
  <tr>
    <td style="background:#1F1B14;border-radius:0 0 12px 12px;padding:20px 32px;text-align:center;">
...
export async function handler(event) {
  ...
  try {
    const { cart, customerName, customerEmail, customerPhone, deliveryAddress, notes, chatHistory } = JSON.parse(event.body);
    ...
    if (!resendKey || resendKey === "re_your-key-here") {
      ...
      return {
        ...
        body: JSON.stringify({ success: true, orderId, demo: true }),
      };
    }

    // ── Send via Resend ──────────────────────────────────────────────────────
    const emailSubject = `MENY Bestilling ${orderId} — ${customerName || "Ny bestilling"} — ${grandTotal} kr`;
